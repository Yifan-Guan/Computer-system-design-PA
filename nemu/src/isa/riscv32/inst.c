...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, sword_t t=src1; if(rd){R(rd)=CSR(imm);} CSR(imm)=t);
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, sword_t t=src1; if(rd){R(rd)=CSR(imm);} CSR(imm)= CSR(imm) | t);
